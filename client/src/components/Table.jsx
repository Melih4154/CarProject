...
const TableComponent = ({ data }) => {
    ...
    const sortByName = () => {
        const usersCopy = [...users];
        usersCopy.sort((userA, userB) => {
            const fullNameA = `${userA.user_id.full_name}`;
            const fullNameB = `${userB.user_id.full_name}`;
            if (sorted.reversed) {
                        ...
                        <tr>
                            <th className='p-3 text-sm tracking-wide font-semibold text-left'>
                                <div className="flex space-x-1">
                                    <span className="inline-flex items-center gap-x-2">Dosya No</span>
                                    <span onClick={sortById}>{sorted.sorted === "id" && sorted.reversed ? <FaSortUp size={14} /> : <FaSortDown size={14} />}</span>
                                </div>
                            </th>
                            <th className='p-3 text-sm tracking-wide font-semibold text-left '>
                                <div className="flex space-x-1">
                                    <span className="inline-flex items-center gap-x-2">İsim Soyisim</span>
                                    <span onClick={sortByName}>{sorted.sorted === "name" && sorted.reversed ? <FaSortUp size={14} /> : <FaSortDown size={14} />}</span>
                                </div>
                            </th>
                            <th className='p-3 text-sm tracking-wide font-semibold text-left'>
                            ...
                            <th className='p-3 text-sm tracking-wide font-semibold text-left'>
                                <div className="inline-flex items-center gap-x-2">Kayıt Tarihi</div>
                            </th>
                            <th className='p-3 text-sm tracking-wide font-semibold text-left'>
                                <div className="inline-flex items-center gap-x-2">Kaza Tarihi</div>
                            </th>
                            <th className='p-3 text-sm tracking-wide font-semibold text-left'>
                                <div className="inline-flex items-center gap-x-2">Dosya Konusu</div>
                            </th>
                            <th className='p-3 text-sm tracking-wide font-semibold text-left'>
                    ...
                    <tbody className='divide-y divide-gray-100'>
                        {users.map((item) => (
                            <tr className="group bg-white" key={item._id}>
                                ...
                                >
                                    <div className="flex gap-x-2.5">
                                        <Link to={`/demage/${item._id}`} state={item}> <div className="h-8 px-4 flex items-center justify-center rounded bg-green-600 text-white">Detay</div></Link>
                                    </div>
                                </td>
                            </tr>
                        ))}
                ...
                {users.map((item, key) => (
                    <div className="bg-gray-600 p-4 rounded-lg shadow space-y-1 text-white font-semibold text-sm">
                        <div className="items-center space-y-3 text-sm">
                            ...
                            <div className="flex gap-x-2.5">
                                <span>T.C.No:</span>
                                <span>{item.user_id.id_number}</span>
                            </div>
                            <div className="flex gap-x-2.5">
                                <span>Plaka:</span>
